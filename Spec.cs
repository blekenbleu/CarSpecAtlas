...
namespace blekenbleu
{
	...
	public class ListDictionary : NotifyPropertyChanged
	{
        ...
        internal string SetGame(string GameDBText, Dictionary<string, List<CarSpec>> json)
		{
			inDict = json;
			int ct = inDict.TryGetValue(GameDBText, out List<CarSpec>? value) ? value.Count : 0;

			return $"{inDict.Count} games, {ct} {GameDBText} cars in ";
		}

		internal void AddCar(string g, CarSpec car)			// ListDictionary: S.LD.AddCar; update Save
		{
			if (inDict.TryGetValue(g, out List<CarSpec>? value))
			{
				int idx = value.FindIndex(x => x.id == car.id);

				if (0 <= idx)
                    value[idx] = car;				// ListDictionary:  replace car in dictionary
				else
                    value.Add(car);				// ListDictionary:  add car to current dictionary
			}
			else inDict.Add(g, [car] );			// ListDictionary:  add new dictionary with car
			return;
		}

		internal int CarCount(string g) { return inDict.TryGetValue(g, out List<CarSpec>? value) ? value.Count : 0; }
...
/*
	public class Spec : NotifyPropertyChanged
	{
		...
		private string? GameDBText;

		public Spec(string gt)
		{
			Private_Car = new() { id = ""};             // required 24 May 2024
            DfltCar = new() { id = "" };
		...
		internal void CarId(string Id)						// store CarID until Set()
		{
			Private_Car.id = Id;
		}
...
//			IdleRPM = (ushort)(Lcache[i].idlerpm = rpm);
			Lcache[i].idlerpm = rpm;
		}
		...
		internal int Cache(string gt, CarSpec c)
		{
			int Idx;
		...
		internal int SelectCar(string gt, List<CarSpec> Atlas, ushort r, ushort m, ushort I)
		{
			GameDBText = gt;
			...
			if (null == DfltCar)
			{
				...
				switch (Haptics.CurrentGame)
				{
					...
					case GameId.WRC23:
						...
						break;
					default:
						StatusText += $"specs unavailable for {GameDBText}";
						break;
				}
#endif
		...
		public ushort Redline
		{
			get => (ushort)Private_Car.redline;
		...
		{
			get => (ushort)Private_Car.idlerpm;						// IdleRPM
